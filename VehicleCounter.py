...
class VehicleCounter():
    ...
    def visualize(self):
        ...
        if self.counting_line != None:
            cv2.line(frame, self.counting_line[0], self.counting_line[1], (255, 0, 0), 2)
        ...
        cv2.putText(frame, 'Count: ' + str(self.vehicle_count) + types_counts_str, (20, 60), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        # cv2.putText(frame, 'Processing speed: ' + str(self.processing_frame_rate) + ' FPS', (20, 120), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        # show detection roi
        if self.show_droi:
